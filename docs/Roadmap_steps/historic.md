...
3. [ ] **IDs serveurs opaques** — Introduire un identifiant stable dérivé (`server_id_hash = blake3(hostname+salt)`) stocké avec les métadonnées. Les endpoints history consomment uniquement cet ID pour éviter les séquences devinables.
...
5. [ ] **Aggregation adaptative** — Ajouter dans `history::query` un mode downsampling (par bucket de taille dynamique) dès que la fenêtre demandée dépasse `history.max_window_seconds/4`. Retourner min/max/avg par bucket pour respecter « granularité dégradée ».
